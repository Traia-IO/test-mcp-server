...
This is an MCP (Model Context Protocol) server that provides access to the test API. It enables AI agents and LLMs to interact with test through standardized tools.

## Features

- 🔧 **MCP Protocol**: Built on the Model Context Protocol for seamless AI integration
- 🌐 **Full API Access**: Provides tools for interacting with test endpoints
...
1. Clone this repository:
   ```bash
   git clone https://github.com/Traia-IO/test-mcp-server.git
   cd test-mcp-server
   ```
...
1. Create API client functions for test endpoints
