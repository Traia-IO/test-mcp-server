#!/usr/bin/env python
"""
test MCP Server Health Check Script

This script properly connects to the test MCP server and checks its health by:
...
def check_mcp_server_health(url: str) -> bool:
    """Check if test MCP server is healthy"""
    print(f"🔍 Checking test MCP server health at {url}")
    ...
    if "error" in result and "session" not in str(result.get("error", "")):
        ...
        if "result" in result and "tools" in result["result"]:
            ...
            if missing_tools:
                ...
                print("✅ All expected test tools are available!")
...
def main():
    parser = argparse.ArgumentParser(description="Check test MCP Server Health")
    parser.add_argument("--url", default="http://localhost:8000", 
                       help="test MCP server URL (default: http://localhost:8000)")
    ...
    if check_mcp_server_health(args.url):
        print("\n✅ test MCP Server is healthy and responding!")
        return 0
    else:
        print("\n❌ test MCP Server health check failed!")
        return 1
