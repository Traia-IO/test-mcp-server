#!/usr/bin/env python3
"""
test MCP Server - FastMCP with D402 Transport Wrapper
...
logger.info("="*80)
logger.info(f"test MCP Server (FastMCP + D402 Wrapper)")
logger.info(f"API: https://pro-api.coinmarketcap.com/")
logger.info(f"Payment: {SERVER_ADDRESS}")
logger.info("="*80)

# Create FastMCP server
mcp = FastMCP("test MCP Server", host="0.0.0.0")
...
# - TEST_API_KEY: Server's internal API key for payment mode
#
...
def create_app_with_middleware():
    ...
    app.add_middleware(
        ...
        server_name="test-mcp-server"  # MCP server ID for tracking
    )
...
if __name__ == "__main__":
    logger.info("="*80)
    logger.info(f"Starting test MCP Server")
    logger.info("="*80)
